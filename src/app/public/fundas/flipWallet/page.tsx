...
function FlipWalletContent() {
  const { selectedMarca } = useFilters();
  const [categorias, setCategorias] = useState<categorias[]>([]);
  const [loading, setLoading] = useState(true);
  const subcategoriaId = 8;
              ...
              {categorias.length === 0 && (
                <div className="text-center py-12">
                  <p className="text-gray-500 text-lg">
                    {selectedMarca 
                      ? `No hay fundas Flip Wallet de ${selectedMarca.nombre} disponibles en este momento.`
                      : 'No hay fundas Flip Wallet disponibles en este momento.'
                    }
                  </p>
                </div>
              )}
...
export default function FlipWallet() {
  return (
    <FiltersProvider>
      <FlipWalletContent />
    </FiltersProvider>
